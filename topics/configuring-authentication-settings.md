...
The module relies on the [SAML Authentication plugin](https://github.com/morincer/teamcity-plugin-saml) bundled with TeamCity since version 2021.12.

To enable the module, in __Administration | Authentication__:
1. Click __Add module__ and choose the _HTTP-SAML.v2_ type.
2. Save the settings and go to __Administration | SAML Settings__.
3. Copy the _Single Sign-On URL_ value under the _Service Provider Configuration_ section. You will need to paste it in the settings of your SSO application, as described below.

Before configuring SAML settings in TeamCity, you need to create a dedicated application in your SSO provider's dashboard. Here is a quick instruction for Okta:
...
>Alternatively to the manual app configuration, you can download the XML file with the TeamCity configuration in __SAML Settings__ and upload it to your SSO profile.
...
Contains the autogenerated SSO URL of a recipient server, that is TeamCity.

You can also generate the SP metadata XML file and use it to automatically configure the app on the SSO provider's side. Before downloading the file, make sure to preconfigure and save __SAML Settings__ in TeamCity so the XML is updated with the relevant settings.
